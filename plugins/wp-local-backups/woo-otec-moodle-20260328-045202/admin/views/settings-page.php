...
$status_cards = array(
    array(
        ...
        'help'  => $route_connection_ok ? 'La conexion con Moodle responde correctamente.' : 'Revisa URL y token de Moodle.',
    ...
    array(
        'label' => 'Estado Sync',
        'value' => $sync_status_label,
        'help'  => !empty($last_sync['message']) ? (string) $last_sync['message'] : 'Aun no hay una ejecucion registrada.',
        'class' => $route_sync_ok ? 'is-success' : 'is-warning',
    ),
    array(
        'label' => 'Correo',
        'value' => $route_email_ok ? 'Configurado' : 'Pendiente',
        'help'  => $route_email_ok ? 'El remitente y el envio estan listos.' : 'Configura remitente y prueba de correo.',
        'class' => $route_email_ok ? 'is-success' : 'is-warning',
    ),
    array(
        'label' => 'SSO Alumno',
        'value' => $sso_enabled ? 'Activo' : 'Desactivado',
        'help'  => $sso_enabled ? 'El acceso directo para alumnos esta habilitado.' : 'Activalo si usaras acceso directo al campus.',
                ...
                <div class="pcc-card">
                    ...
                    <section class="pcc-tab-panel <?php echo $active_tab === 'inicio' ? 'is-active' : ''; ?>" id="pcc-panel-inicio" data-panel="inicio" role="tabpanel">
                        <div class="pcc-home-hero">
                            <div>
                                <h2>Inicio</h2>
                                <p class="description">Desde aqui puedes ver el estado general del plugin y comenzar rapido con el asistente de configuracion.</p>
                            </div>
                            <div class="pcc-home-hero__actions">
                                <a href="<?php echo esc_url(admin_url('admin.php?page=woo-otec-asistente')); ?>" class="button button-primary button-large">Iniciar Asistente de Configuracion</a>
                            </div>
                        </div>
                        ...
                        <div class="pcc-overview-grid">
                            <?php foreach ($status_cards as $card) : ?>
                                <div class="pcc-overview-card <?php echo esc_attr((string) $card['class']); ?>">
                                    ...
                                    <p><?php echo esc_html((string) $card['help']); ?></p>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    </section>
