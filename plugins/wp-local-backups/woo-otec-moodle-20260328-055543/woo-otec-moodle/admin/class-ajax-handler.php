...
final class Woo_OTEC_Moodle_Ajax_Handler {
    ...
    private function build_template_preview_html(int $product_id, array $selected_fields): string {
        ...
        foreach ($selected_fields as $meta_key) {
            ...
            if ($image_url !== '') {
                $caption = $this->get_template_preview_image_caption($value);

                $html .= '<div class="pcc-template-live-item is-image">';
                $html .= '<span>' . esc_html($label) . '</span>';
                $html .= '<strong>Vista previa de la imagen sincronizada</strong>';
                $html .= '<div class="pcc-template-live-image-wrap">';
                $html .= '<img src="' . esc_url($image_url) . '" alt="' . esc_attr($label) . '">';
                $html .= '</div>';
                $html .= '<small>' . esc_html($caption) . '</small>';
                $html .= '</div>';
                continue;
            }

            if ($value === '') {
                $value = 'Sin valor disponible en este curso';
            } else {
    ...
    private function get_template_preview_image_caption(string $value): string {
        $raw_value = trim($value);

        if ($raw_value === '') {
            return 'Se muestra la portada actual del curso.';
        }

        if (str_starts_with($raw_value, 'manual:')) {
            return 'Imagen seleccionada manualmente desde WordPress.';
        }

        if (str_starts_with($raw_value, 'default:')) {
            return 'Imagen por defecto usada cuando Moodle no trae portada.';
        }

        if (filter_var($raw_value, FILTER_VALIDATE_URL)) {
            return 'Imagen obtenida directamente desde Moodle.';
        }

        return 'Portada disponible para este curso.';
    }
    ...
    public function handle_import_config(): void {
        ...
        if (!is_array($decoded)) {
            wp_send_json_error(array('message' => 'El archivo no contiene un JSON valido.'), 400);
        }

        $settings = $this->extract_import_settings($decoded);
        if (!is_array($settings) || empty($settings)) {
            wp_send_json_error(array('message' => 'El archivo no tiene el formato esperado o no contiene configuracion importable.'), 400);
        }
