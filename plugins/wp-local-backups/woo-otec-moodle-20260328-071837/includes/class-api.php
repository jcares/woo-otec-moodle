...
final class Woo_OTEC_Moodle_API {
    ...
    private function request(string $function, array $params = [], array $args = []): mixed {
        ...
        if (is_wp_error($response)) {
            ...
            if (strpos($error_msg, 'Could not resolve host') !== false) {
                ...
                $friendly = 'Error de Tiempo de Espera: El servidor de Moodle tardo demasiado en responder.';
            } else {
                $friendly = $error_msg;
            }
        ...
        if ($status_code < 200 || $status_code >= 300) {
            ...
            throw new PCC_Moodle_Exception(
                'Respuesta HTTP invalida desde Moodle (codigo ' . $status_code . ').',
                'pcc_http_error',
                $function,
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            ...
            throw new PCC_Moodle_Exception('JSON invalido desde Moodle.', 'pcc_invalid_json', $function);
        }
        ...
        if (is_object($decoded) && (isset($decoded->exception) || isset($decoded->errorcode))) {
            ...
            Woo_OTEC_Moodle_Logger::error('Excepcion Moodle', [
                'function'  => $function,
    ...
    /**
     * Wrapper publico: captura PCC_Moodle_Exception y devuelve WP_Error.
     * Usar en metodos que exponen la API al resto del plugin.
     */
    ...
    public function get_course_teachers(int $course_id): array {
        try {
            ...
            if ($e->get_error_code() === 'pcc_moodle_exception') {
                Woo_OTEC_Moodle_Logger::error('Permisos insuficientes para obtener profesores. Verifica que core_enrol_get_enrolled_users este habilitada en Moodle.', ['course_id' => $course_id]);
            }
            return [];
        }
